 ...
 * секунде и длится РОВНО 10 секунд — при скорости 1.0.
 * Два разворота на 180° (блок-свапа): ходы 6-й и 16-й (i==5 и i==15),
 * второй начинается примерно через 3 секунды после первого.
 * Ходов теперь 38 (вместо 26), каждый быстрее (230 мс) — уследить за
 * верным ключом труднее:
 *   t1 = 5*230 = 1150 мс;  t2 = 1150 + 900 + 9*230 = 4120 мс (~3 с).
 *   Итого: 35 ходов * MOVE_MS + 2 * SPECIAL_MOVE_MS + FINAL_MOVE_MS = 10000. */
#define SHUFFLE_START_AT_MS   5000
#define SHUFFLE_TOTAL_MS      10000
#define MOVE_MS               230     /* 35 * 230 = 8050 */
